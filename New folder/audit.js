...
function parse(html) {
  try { return new JSDOM(html).window.document; } catch { return null; }
}
...
function extractData(doc) {
  ...
  // CONTACT EMAIL — hidden input in the venue enquiry form
  const emailInput = doc.querySelector('.cta8-form-wrap.is-venue input[type=hidden]')
    || doc.querySelector('.cta8-form-wrap input[type=hidden]');
  const email = emailInput ? (emailInput.value || '').trim() : '';

  // SOCIAL LINKS — href="#" means the venue never filled it in
  const socialHrefs = [...new Set(
    Array.from(doc.querySelectorAll('.social-icons-2 .cta6-icon')).map(a => (a.getAttribute('href') || '').trim())
  )].filter(h => h && h !== '#');
  const hasFacebook = socialHrefs.some(h => /facebook\.com/i.test(h));
  const hasTwitter = socialHrefs.some(h => /twitter\.com|(?:^|\.)x\.com/i.test(h));
  const hasInstagram = socialHrefs.some(h => /instagram\.com/i.test(h));
  const socialCount = [hasFacebook, hasTwitter, hasInstagram].filter(Boolean).length;
  ...
  return {
    ...
    email, hasFacebook, hasTwitter, hasInstagram, socialCount, website, hasPhone,
...
function auditVenue(venue, doc) {
  ...
  if (!d.hasFacebook) missingSocials.push('Facebook');
  if (!d.hasTwitter) missingSocials.push('Twitter/X');
  if (!d.hasInstagram) missingSocials.push('Instagram');

  if (!d.email && d.socialCount === 0 && !d.website) {
    issues.push(issue('contact', 'critical',
      'No contact information at all — no email, no social media, no website link',
      'Add at minimum a contact email and the venue\'s website and social media links so enquirers can reach the venue directly'));
    score -= 15;
  } else {
    if (!d.email) {
      issues.push(issue('contact', 'warning',
        'No contact email address behind the enquiry form',
        'Set the venue\'s enquiry email so the contact form reaches the venue (currently enquirers can only use the site-wide form)'));
      score -= 10;
    }
    if (missingSocials.length === 3) {
      issues.push(issue('contact', 'warning',
        'No social media links — all three social icons are unfilled placeholders',
        'Add the venue\'s Facebook, Twitter/X and Instagram profile URLs'));
      score -= 6;
    } else if (missingSocials.length > 0) {
      issues.push(issue('contact', 'warning',
        `Missing social media link(s): ${missingSocials.join(', ')} (shown as a dead "#" link)`,
        `Add the venue's ${missingSocials.join(' and ')} profile URL(s) — the placeholder "#" links look broken to visitors`));
      score -= 4;
    }
    if (!d.website) {
  ...
  return {
    ...
    hasAccommodation,
    hasAccessibility,
    hasContact: !!d.email || d.socialCount > 0,
    hasEmail: !!d.email,
    hasPhone: d.hasPhone,
    hasWebsite: !!d.website,
    hasFacebook: d.hasFacebook,
    hasTwitter: d.hasTwitter,
...
async function main() {
      ...
      try {
        ...
        if (!isVenuePage) {
          ...
          return {
            ...
            hasEmail: false, hasPhone: false, hasWebsite: false,
            hasFacebook: false, hasTwitter: false, hasInstagram: false,
  ...
  const categories = ['description', 'capacity', 'photos', 'facilities', 'parking', 'catering', 'accommodation', 'accessibility', 'contact'];
  ...
  // Summary CSV
  const summaryHeaders = ['Venue Name', 'URL', 'Score', 'Severity', 'Unclaimed Listing', 'Description Length', 'Photo Count', 'Amenity Count', 'Has Capacity', 'Has Parking', 'Has Catering', 'Has Accommodation', 'Has Accessibility', 'Has Email', 'Has Phone', 'Has Website', 'Has Facebook', 'Has Twitter/X', 'Has Instagram', 'Room Count', 'Issue Count', 'Critical Issues', 'Warnings', 'Suggestions'];
  const summaryRows = results.map(v => [
    ...
    v.hasEmail ? 'Yes' : 'No', v.hasPhone ? 'Yes' : 'No', v.hasWebsite ? 'Yes' : 'No',
    v.hasFacebook ? 'Yes' : 'No', v.hasTwitter ? 'Yes' : 'No', v.hasInstagram ? 'Yes' : 'No',
  ...
  fs.writeFileSync(path.join(process.cwd(), 'kentvenues-audit-summary.csv'), '\ufeff' + summaryCsv, 'utf8');
  console.log(`\n  Summary CSV: kentvenues-audit-summary.csv`);
  ...
  fs.writeFileSync(path.join(process.cwd(), 'kentvenues-issues-detailed.csv'), '\ufeff' + issuesCsv, 'utf8');
  console.log(`  Issues CSV:   kentvenues-issues-detailed.csv`);

  // JSON for the interactive viewer
  fs.writeFileSync(path.join(process.cwd(), 'kentvenues-audit-data.json'),
  ...
  console.log(`  JSON Data:    kentvenues-audit-data.json`);
...
function generateHTMLReport(results, critical, warnings, good, avgScore, categories) {
  ...
  const rows = sorted.map(v => {
    ...
    return `<tr>
      ...
      <td>${v.hasEmail ? '✓' : '✗'}${v.hasFacebook ? ' fb' : ''}${v.hasTwitter ? ' tw' : ''}${v.hasInstagram ? ' ig' : ''}</td>
...
<div class="container">
  ...
  fs.writeFileSync(path.join(process.cwd(), 'kentvenues-audit-report.html'), html, 'utf8');
}
